...
SELECT ac.account_type, ac.name, ts.balance
FROM T AS ts
LEFT JOIN accounts AS ac ON ts.account_id = ac.account_id
WHERE ts.rn = 1 AND
...
CREATE OR REPLACE FUNCTION get_month_balance(a_month integer, a_account_type integer) RETURNS SETOF month_balance AS $$
    SELECT ac.name,
    CASE a_account_type
    WHEN 1 THEN coalesce(ts.cash_accum_diff, 0)
    WHEN 2 THEN coalesce(-ts.cash_accum_diff, 0)
    WHEN 3 THEN coalesce(ts.accrual_credit_amount, 0) - coalesce(ts.accrual_debit_amount, 0)
    WHEN 4 THEN coalesce(ts.accrual_debit_amount, 0) - coalesce(ts.accrual_credit_amount, 0)
    END AS amount
    FROM accounts AS ac
...
CREATE OR REPLACE FUNCTION get_month_income(a_month integer) RETURNS SETOF month_balance AS $$
    SELECT * FROM get_month_balance(a_month, 3);
$$ LANGUAGE SQL;
...
CREATE OR REPLACE FUNCTION get_month_expense(a_month integer) RETURNS SETOF month_balance AS $$
    SELECT * FROM get_month_balance(a_month, 4);
$$ LANGUAGE SQL;
